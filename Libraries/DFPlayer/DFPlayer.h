...
#define PLAYLIST			0x21 // Not supported by YX5200-24SS but YX6300-24SS
...
inline void send() {
	...
	/*
	 ...
	 or (this->fifoCount % DFPLAYER_BUFFER_LENGTH != 0))) {
...
/*
 // NOT SUPPORTED BY YX5200-24SS chip
inline void playList() {
	//This one is special since its send buffer size can be extended

	//Prouf of concept
	uint8_t sendBuffer[] { 0x7E, 0xFF, 0x07, 0x21, 0x02, 0x01, 0x02, 0x02, 0xFF,
			0xD7, 0xEF };
	this->mp3Serial->write(sendBuffer, 11);
	send();
...
inline uint16_t playTrack(uint16_t value) {
...
inline void playMode(uint8_t value) {
	setSendBuffer(PLAY_MODE, value);
	send();
}
inline uint16_t play() {
