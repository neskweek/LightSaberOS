...
//				Serial.println(prevOrientation.z);
    ...
    if (PrevSaberState==S_STANDBY) { // entering config mode
      ...
      neopixels_stripeKillKey_Disable();
		...
		case 0: // SOUNDFONT
			confMenuStart(storage.soundFont, 5);
    ...
    case 1: //VOLUME
      confMenuStart(storage.volume, 4);
  ...
  dfplayer.setVolume(storage.volume); // Too Slow: we'll change volume on exit
      ...
      break;
			...
			confMenuStart(storage.sndProfile[storage.soundFont].mainColor, 6);
			...
			confMenuStart(storage.sndProfile[storage.soundFont].clashColor, 7);
...
/*NEOPIXEL*/
      ...
      confMenuStart(storage.sndProfile[storage.soundFont].blasterboltColor, 8);
...
#ifdef LEDSTRINGS
	lightFlicker(ledPins, soundFont.getFlickerEffect());
#endif
...
#if defined NEOPIXEL
			if (not isFlickering) {
				getColor(storage.sndProfile[storage.soundFont][1]);
				lightOn(currentColor);
			}
#endif
#if defined FoCSTRING
...
#if defined NEOPIXEL
			if (not isFlickering) {
				getColor(storage.sndProfile[storage.soundFont][0]);
				lightOn(currentColor);
			}
#endif
#if defined FoCSTRING
...
//			}
	...
	} else if (ActionModeSubStates!=AS_BLADELOCKUP && randomBlink != 0) { // We have released lockup button
#if defined FoCSTRING
			FoCOff(FoCSTRING);
#endif
#if defined LUXEON
